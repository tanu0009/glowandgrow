...
$webpagetitle=$pagetitle="Scholarship";
include("header.php");
                                ...
                                <div class="form-group">
									...
									<select class="form-control" name="user_id" id="user_id" >
										<option value="">Select</option>
										 <option value="ALL" id="all_1">All Student</option>
											<?php $listta="SELECT a.sch_id as mem_id,a.stud_name as name, 'Active' as mem_status,CONCAT_WS(' ', c.first_name, c.middle_name, c.last_name, c.mem_no) AS `namex` FROM lad_mst_scholar a, lad_trn_member b, lad_member c WHERE a.active='y'  AND a.sch_id=b.sub_scheme_id AND b.schem_id='sch' AND b.mem_id=c.mem_id AND (a.stud_name like '".$_GET['term']."%' OR c.last_name like '".$_GET['term']."%' OR c.first_name like '".$_GET['term']."%' OR c.mem_no like '".$_GET['term']."%')";
...
function askForbill() {
	...
	{   
		...
		if(reto=='HT'){
			form.action = "scholarship-pdf.php?report_type="+report_type+"&user_id="+user_id+"&fdate="+fdate1+"&tdate="+tdate1;
		}else if(reto=='EX'){
			form.action = "truckexcel.php?report_type="+report_type+"&user_id="+user_id+"&fdate="+fdate1+"&tdate="+tdate1;
		}
		form.submit();
